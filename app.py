...
def main():
    ...
    st.write("**Please press the button above to reset the session state. This is because the app is hosted on Streamlit Cloud and the session state is not reset automatically. After every run, press the x on the video uploaded and press this button again. If pressing the button causes an error on the page, simply refresh the page.**")
